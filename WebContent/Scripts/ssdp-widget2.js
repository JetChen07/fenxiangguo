...
(function (window, $) {
    ...
    ssdp.dialog.prototype = {
        ...
        _create: function () {
            ...
            this.$closeBottom = $html.find("#global-dialog-closeBottom");

            this.modal = new ssdp.modal();
        },
        _setParams: function () {
            var _this = this;
            this.$titleWrap.html(this.title);
            this.$contentWrap.width(this.width);
            this.$contentWrap.height(this.height);
            document.getElementById("global-dialog-content").innerHTML=this.content;
            //this.$contentWrap.html(this.content);
            this.$wrap.width(this.wrapwidth);
            this.$wrap.height(this.wrapheight);
        ...
        _bindEvent: function () {
            ...
            _this.$closeBottom[0].onclick = function () {
                _this.closeFn && _this.closeFn();
                _this.close();
            };
        },
        ...
        setPosition: function () {
            ...
            var left = (($(window).width() - this.$wrap.width()) / 2 + 170);
            var top = (($(window).height() - this.$wrap.height()) / 2);
            this.$wrap.css({
                "position": "fixed",
                "z-index": this.zIndex,
    ...
    ssdp.tip.prototype = {
        _create: function () {
            ...
            this.$wrap = $(html);
            this.$wrap.hide();
            this.$wrap.find("p").text(this.message);
            $(document.body).append(this.$wrap);
            var left = (($(window).width() - this.$wrap.width()) / 2);
            var top = (($(window).height() - this.$wrap.height()) / 2);
            this.$wrap.css({
            ...
            if (this.type === "fail"){
                this.$wrap.removeClass("success");
                this.$wrap.addClass("error");
            } else {
                this.$wrap.removeClass("error");
                this.$wrap.addClass("success");
            }
        },
        open: function () {
            var _this = this;
    ...
    function getCompontentHtml(selector) {
        if (!$widgetHtml) {
            ...
            $widgetHtml = '<div class="ui-modal" id="global-dialog"><button type="button" class="ui-btn ui-btn-link" id="global-dialog-closeButton">&times;</button><div class="ui-modal-header" id="global-dialog-drag"><h3 id="global-dialog-title">标题</h3></div><div class="ui-modal-body" id="global-dialog-content"></div><div class="ui-modal-footer" id="global-dialog-footer"><a href="javascript:;" class="ui-btn ui-btn-primary ui-btn-wid" id="global-dialog-yesButton">确定</a><a href="javascript:;" class="ui-btn ui-btn-wid" id="global-dialog-closeBottom">关闭</a></div></div><div id="global-modal" class="ui-mask"></div>'
